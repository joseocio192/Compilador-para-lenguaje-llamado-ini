...
public class Scanner {
  ...
  private boolean isFloatKeyword(String source, int posicion) {
    return source.substring(posicion, Math.min(posicion + 5, source.length())).equals("float")
        && (Character.isWhitespace(source.charAt(posicion + 5)));
  }
  ...
  private boolean isStringKeyword(String source, int posicion) {
    return source.substring(posicion, Math.min(posicion + 6, source.length())).equals("string")
        && (Character.isWhitespace(source.charAt(posicion + 6)) || source.charAt(posicion + 6) == '(');
  }
  ...
  private boolean isIniKeyword(String source, int posicion) {
    return source.substring(posicion, Math.min(posicion + 3, source.length())).equals("ini")
        && (Character.isWhitespace(source.charAt(posicion + 3)) || source.charAt(posicion + 3) == '{');
  }

  private boolean isIfKeyword(String source, int posicion) {
    return source.substring(posicion, Math.min(posicion + 2, source.length())).equals("if")
        && (Character.isWhitespace(source.charAt(posicion + 2)) || source.charAt(posicion + 2) == '(');
  }

  private boolean isElseKeyword(String source, int posicion) {
    return source.substring(posicion, Math.min(posicion + 4, source.length())).equals("else")
        && (Character.isWhitespace(source.charAt(posicion + 4)) || source.charAt(posicion + 4) == '{');
  }

  private boolean isMostrarKeyword(String source, int posicion) {
    return source.substring(posicion, Math.min(posicion + 7, source.length())).equals("mostrar")
        && (Character.isWhitespace(source.charAt(posicion + 7)) || source.charAt(posicion + 7) == '(');
  }
